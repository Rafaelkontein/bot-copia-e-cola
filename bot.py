class ChromeAuto:
    ...
    def clica_perfil(self):
        try:
            perfil = self.chrome.find_element_by_css_selector(  # aqui ta pegando o seletor para clicar
                '#huro-app > div.view-wrapper > div > div > div.page-content-inner > div > div.onboarding-wrap > div > '
                'div:nth-child(2) > div > a.button.h-button.is-primary.is-outlined.is-rounded.is-raised')
            perfil.click()
        except Exception as e:
    ...
    def faz_login(self):
        try:
            ...
            input_login.send_keys('')  # aqui voce está pedindo para escrever no login
            input_password.send_keys('')  # Aqui voce está falando para escrever a senha
            sleep(3)
            btn_login.click()
        except Exception as e:
